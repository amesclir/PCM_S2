...
Desafortunadamente, debido a la semejanza de organismos vivos relacionados que descienden de ancestros comunes en el pasado, la suposición de independencia de los valores de las variables generalmente no se cumple para los datos que se originan en especies diferentes (Felsenstein 1985). Por el contrario, de hecho, las especies más estrechamente relacionadas a menudo tienden a tener desviaciones residuales correlacionadas en sus valores de y con respecto a los predichos por el modelo. Por ejemplo, si una especie tiene una gran desviación residual positiva en y en comparación con el valor predicho, entonces es más probable que otras especies hermanas estrechamente relacionadas tengan grandes valores residuales positivos también. Si este es el caso, entonces los errores residuales en y están correlacionados, lo que implica una violación del supuesto del método de mínimos cuadrados de que los errores residuales están distribuidos de manera identica e independiente (i.i.d). Para el caso de la regresión lineal, la técnica de contrastes filogenéticamente independientes resuelve este problema transformando primero los datos en un conjunto de contrastes que son teóricamente independientes de la filogenia y por lo tanto no violan i.i.d. Esto significa que podemos proceder a utilizar estos contrastes con el método de los mínimos cuadrados ordinarios.
...
Lo primero que haremos en este capítulo es "probar" la equivalencia de la regresión de contrastes (es decir, PIC), sobre la cual aprendimos en el anteriormente, y PGLS. Lo que podemos hacer es simplemente ajustar una regresión de contrastes y una regresión PGLS a los mismos datos y luego mostrar que se obtiene un resultado numéricamente idéntico. Para ilustrar esto, usaremos un análisis de la morfología de la órbita de los primates. Podemos relacionar el tamaño de los ojos con el tamaño del cráneo y, eventualmente, con los patrones de actividad diaria de las especies. Este ejemplo proviene de un artículo de Kirk y Kay (2004), ampliado en una maravillosa publicación de blog de Randi Griffin (2017). Los archivos de datos (primateEyes.csv) y los archivos de árbol (primateEyes.phy) están disponibles en este proyecto de R.

Como queremos comparar la regresión PIC con PGLS, comencemos cargando nuestros datos y el árbol, calculando contrastes independientes usando pic y luego ajustando un modelo de regresión lineal a nuestros contrastes usando lm. Luego podemos repetir nuestro análisis usando PGLS. Este análisis simplemente duplica lo que ya hemos aprendido.
...
La pendiente del modelo ajustado es muy significativa y tiene un valor estimado de aproximadamente 1,38. Dado que ambos rasgos están transformados logarítmicamente, esta pendiente proporciona la relación alométrica que mejor se ajusta entre la longitud del cráneo y el área de la órbita en todos los primates de nuestros datos. Para visualizar esto, ¿por qué no trazamos también nuestros datos originales más nuestros PIC y la regresión PIC de una manera similar a lo que vimos la última vez?
...
Ahora vamos a proceder con nuestro PGLS. Para ello vamos a usar el paquete nlme.
Para nuestro siguiente paso, necesitamos tomar nuestro árbol filogenético de primates y convertirlo en un tipo especial de objeto R llamado estructura de correlación. Esto es lo que nuestra función de ajuste de modelos GLS tomará como entrada, junto con nuestros datos. Luego, la estructura de correlación se usará para definir la distribución de los residuos de nuestro modelo lineal. Por ahora, construiremos nuestro objeto "corStruct" usando la función mono corBrowniana.
...
Ahora estamos listos para ajustar nuestro modelo lineal. Haremos esto usando la función gls de  nlme. gls funciona de manera muy similar a la función lm que ya hemos aprendido pero (para nuestros propósitos) tomaremos solo un argumento adicional: correlation. El valor de la correlación es solo nuestro "corStruct", corBM, que generamos anteriormente usando corBrownian.
...
¿Cómo se compara esto con el modelo que ajustamos a nuestros contrastes anteriormente? Son, como mínimo, similares, pero ¿son idénticos? Para averiguarlo. Podríamos hacer esto usando la prueba lógica ==; sin embargo, en las computadoras suele ser una buena idea evitar intentar evaluar la igualdad de los números reales. Esto se debe a que, por razones técnicas, los números reales pueden diferir en el decimoquinto decimal incluso cuando se supone que son idénticos. En lugar de eso, calculemos el valor absoluto de la diferencia entre los coeficientes de pendiente de los dos modelos. Si esto se evalúa como un número muy pequeño, entonces sugiere que sus valores son los mismos.
...
Una diferencia clara que se puede apreciar entre PIC y PGLS es que en la seguna hay una ordenada al origen (intercept o intercepto) pero no en el PIC. Esto es poque en PGLS no tranformamos nuestros datos en un nuevo especio previo análisis y por lo tanto no estamos forzados a renunciar a este parámetro en nuestro modelo.
...
En resumen podemos concluir que tal y como fue demostrado anteriormente (Blomberg et al., 2012) PIC es simplemente un caso especial de PGLS. Concretamente al caso en que se asume que la estructura de la correlación de errores residuales del modelo corresponde a un model Browniano.

Hemos la estructura de correlación de error residual más simple para datos filogenéticos, que se llama corBrowniana. Como señalamos anteriormente, esta estructura simplemente supone que la correlación entre los errores residuales de cualquier par de especies en el árbol es directamente proporcional al tiempo en el árbol que evolucionan conjuntamente desde la raiz (cuando son el mismo linaje). Esta correlación esperada surge directamente de un modelo implícito que hemos asumido con respecto a cómo evolucionan nuestros rasgos de carácter en el árbol. 
...
Es relativamente fácil entender cómo la matriz de correlación se completa a partir de la filogenia. Tomemos, por ejemplo, la correlación entre los taxones C y E. Para obtener esto, simplemente dividimos la distancia desde la raíz del común ancestro de C y E (4.36) por la longitud total del árbol (10) y obtenemos la correlación (0.44). Asimismo, para obtener la correlación entre los taxones C y D, calculamos la distancia desde la raíz hasta su ancestro común (4.36 + 2,96 = 7,32) y dividirlo por la longitud total del árbol (10) para obtener la correlación, 20 la cual (redondeada a dos dígitos).
...
Ahora que entendemos la forma de nuestro modelo para la correlación del error residual en PGLS, también es sencillo imaginar formas en las que se podrían relajar los supuestos de este modelo. Una relajación muy simple de nuestro modelo podría ser mediante la introducción de un único parámetro adicional, digamos λ, como multiplicador de los elementos fuera de la diagonal de la matriz (Pagel 1999a). Lo interesante de este modelo varía desde un un OLS (cuando λ = 0) hasta PGLS estándar (cuando λ = 1). Podemos determinar qué valor de λ está mejor respaldado por el patrón en nuestros datos estimándolo usando un procedimiento llamado máxima verosimilitud. 
...
Para ver cómo funciona esto, podemos probar la hipótesis de que el patrón de actividad diaria (nocturno, diurno o catémeral) afecta el tamaño relativo del ojo. Decimos aquí tamaño relativo del ojo porque nuestro análisis también controlará la alometría al incluir el tamaño del cráneo como una covariable adicional. Este tipo de modelo se llama modelo ANCOVA y asumiremos que la estructura de correlación del error residual está dada por la filogenia. Por lo tanto, llamaremos a nuestro modelo ANCOVA filogenético generalizado. El patrón de actividad ya es una columna en nuestro conjunto de datos primate.data. También hemos construido nuestra estructura de correlación, corBM, que depende sólo de nuestro árbol.
